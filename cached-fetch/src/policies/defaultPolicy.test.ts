...
const spiedPolicy = vi.spyOn(DEFAULT_POLICIES, 'default')
...
it('reuses cache for the 2nd time', async () => {
  ...
  const promise = cachedFetch(input, init)
  expect(spiedPolicy).toHaveBeenCalled()
  expect(spiedFetch).not.toHaveBeenCalled() // cache hit
  expect(cache.get(cacheKey).date).toEqual(cached.date) // cache not updated
